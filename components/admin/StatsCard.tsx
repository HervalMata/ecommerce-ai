    ...
    const content = (
        <div
            className={cn(
                ...
                href && "cursor-pointer transition-colors hover:border-zinc-300 hover:bg-zinc-50 " +
                "dark:hover:border-zinc-700 dark:bg-zinc-800/50",
            )}
        >
...
function StatsCardSkeleton(
    {
        title,
        icon: Icon,
    }: Pick<StatsCardProps, "title" | "icon">
) {
    return (
...
export function StatsCard(props: StatsCardProps) {
    return (
        <Suspense fallback={<StatsCardSkeleton title={props.title} icon={props.icon} />}>
