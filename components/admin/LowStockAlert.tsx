...
import {DocumentHandle, useDocumentProjection, useDocuments} from "@sanity/sdk-react";
        ...
        >
            <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-md bg-zinc-200 dark:bg-zinc-700">
                ...
                <Badge
                    ...
                    className="shrink-0"
                >
                    {isOutOfStock ? "Fora de Estoque" : `${data.stock} restantes`}
                </Badge>
        </Link>
    );
}
...
function LowStockProductRowSkeleton() {
    return (
        <div className="flex items-center gap-5 rounded-lg border border-zinc-100 bg-zinc-50 p-3
        ...
        return (
            <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="text-sm text-zinc-500 dark:text-zinc-400">
                    Todos os produtos tem bom estoque
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-2">
            {lowStockProducts.slice(0, 5).map((handle) => (
                <Suspense key={handle.documentId} fallback={<LowStockProductRowSkeleton />}>
            ...
            {lowStockProducts.length > 5 && (
                <Link
                    ...
                    className="block text-center text-sm text-zinc-500 hover:text-zinc-700 dark:text-zinc-400
                                dark:hover:text-zinc-200"
                >
                    Veja Todos {lowStockProducts.length} itens com baixo estoque →
                </Link>
            )}
        </div>
    )
